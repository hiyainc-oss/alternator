...
trait AlpakkaTableOps[V, PK] extends crud.TableOps[V, PK, Future, Source[*, NotUsed]] {
  ...
  def batchedGet(key: PK)(implicit
  ...
  def batchedGetFlow(parallelism: Int)(implicit
    actorRef: ActorRef[BatchedReadBehavior.BatchedRequest],
    timeout: Timeout,
    scheduler: Scheduler
  ): Flow[PK, Option[DynamoFormat.Result[V]], NotUsed]
  def batchedGetFlowUnordered[PT](parallelism: Int)(implicit
    actorRef: ActorRef[BatchedReadBehavior.BatchedRequest],
    timeout: Timeout,
    scheduler: Scheduler
...
object AlpakkaTableOps {
  ...
  private def sendRead[V](pk: PK, deserializer: AV => DynamoFormat.Result[V])(implicit
    ...
    actorRef
      .ask((ref: BatchedReadBehavior.Ref) => BatchedReadBehavior.Req(pk, ref))
  ...
  final case class ReadRequestWoPT[V](pk: PK, deserializer: AV => DynamoFormat.Result[V])
    extends ReadRequest[Option[DynamoFormat.Result[V]]] {
    def send()(implicit
      actorRef: ActorRef[BatchedReadBehavior.BatchedRequest],
      timeout: Timeout,
      scheduler: Scheduler
  ...
  final case class ReadRequestWPT[V, PT](pk: PK, deserializer: AV => DynamoFormat.Result[V], pt: PT)
    extends ReadRequest[(Option[DynamoFormat.Result[V]], PT)] {
    def send()(implicit
      actorRef: ActorRef[BatchedReadBehavior.BatchedRequest],
      timeout: Timeout,
      scheduler: Scheduler
    ): Future[(Option[DynamoFormat.Result[V]], PT)] = {
