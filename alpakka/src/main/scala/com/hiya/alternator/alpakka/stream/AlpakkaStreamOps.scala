...
class AlpakkaStreamOps[V, PK](val table: Table[V, PK]) {
  ...
  final def putRequest[PT](item: V, pt: PT): AlpakkaTableOps.WriteRequest[PT] = {
    ...
    AlpakkaTableOps.WriteRequest(table.tableName -> table.schema.serializePK(table.schema.extract(item)), Some(itemValue), pt)
  }
  ...
  final def deleteRequest[T, PT](item: T, pt: PT)(implicit T : ItemMagnet[T, V, PK]): AlpakkaTableOps.WriteRequest[PT] = {
