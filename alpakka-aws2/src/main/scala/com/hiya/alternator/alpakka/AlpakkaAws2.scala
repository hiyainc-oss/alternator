...
class AlpakkaAws2(implicit val system: ClassicActorSystemProvider) extends DynamoDB[Future, Source[*, NotUsed], DynamoDbAsyncClient] {
  ...
  override def get[V, PK](table: TableLike[DynamoDbAsyncClient, V, PK], pk: PK): Future[Option[Result[V]]] = {
    table.client.getItem(Aws2Table(table).get(pk).build()).asScala.map(Aws2Table(table).deserialize)(ExecutionContext.parasitic)
  }

  override def put[V, PK](table: TableLike[DynamoDbAsyncClient, V, PK], item: V, condition: Option[ConditionExpression[Boolean]]): Future[Boolean] =
    condition match {
      case Some(condition) =>
        table.client.putItem(Aws2Table(table).put(item, condition).build()).asScala
          .map(_ => true)(ExecutionContext.parasitic)
          .recover { case _: model.ConditionalCheckFailedException => false }(ExecutionContext.parasitic)
      case None =>
        table.client.putItem(Aws2Table(table).put(item).build()).asScala
          .map(_ => true)(ExecutionContext.parasitic)
    }

  override def delete[V, PK](table: TableLike[DynamoDbAsyncClient, V, PK], key: PK, condition: Option[ConditionExpression[Boolean]]): Future[Boolean] =
    condition match {
      case Some(condition) =>
        table.client.deleteItem(Aws2Table(table).delete(key, condition).build()).asScala
          .map(_ => true)(ExecutionContext.parasitic)
          .recover { case _: model.ConditionalCheckFailedException => false }(ExecutionContext.parasitic)
      case None =>
        table.client.deleteItem(Aws2Table(table).delete(key).build()).asScala
          .map(_ => true)(ExecutionContext.parasitic)
    }


  override def createTable(client: DynamoDbAsyncClient, tableName: String, hashKey: String, rangeKey: Option[String], readCapacity: Long, writeCapacity: Long, attributes: List[(String, ScalarType)]): Future[Unit] =
    client.createTable(Aws2Table.createTable(tableName, hashKey, rangeKey, readCapacity, writeCapacity, attributes).build()).asScala.map(_ => ())(ExecutionContext.parasitic)

  override def dropTable(client: DynamoDbAsyncClient, tableName: String): Future[Unit] =
    client.deleteTable(model.DeleteTableRequest.builder().tableName(tableName).build()).asScala.map(_ => ())(ExecutionContext.parasitic)

  override def scan[V, PK](table: TableLike[DynamoDbAsyncClient, V, PK], segment: Option[Segment]): Source[Result[V], NotUsed] = {
    Source
  ...
  override def query[V, PK, RK](table: TableWithRangeLike[DynamoDbAsyncClient, V, PK, RK], pk: PK, rk: RKCondition[RK]): Source[Result[V], NotUsed] =
    Source
  ...
  override def batchGet[V, PK](table: TableLike[DynamoDbAsyncClient, V, PK], keys: Seq[PK]): Future[BatchGetItemResponse] =
