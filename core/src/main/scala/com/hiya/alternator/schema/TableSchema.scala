...
abstract class TableSchema[V](val serializeValue: RootDynamoFormat[V]) {
  ...
  def withName(tableName: String): Table[DynamoDBClient.Missing, V, IndexType] =
    new Table[DynamoDBClient.Missing, V, IndexType](DynamoDBClient.Missing, tableName, this)
