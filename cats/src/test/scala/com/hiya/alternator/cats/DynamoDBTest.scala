...
import com.hiya.alternator.syntax.rk
...
class DynamoDBTest extends AnyFunSpec with Matchers {
  ...
  class ExampleDB(name: String)(implicit val client: DynamoDbAsyncClient)
  {
    ...
    def get(key: String): IO[Option[ExampleData]] = table.get(key).throwErrors
  ...
  describe("query") {
    ...
    it("should compile =") {
      ...
      val result = withRangeData(5) { table =>
        Await.result(table.query(pk = "5", rk == "3").compile.to(List).unsafeToFuture().throwErrors, TEST_TIMEOUT)
      }
    ...
    it("should compile between") {
      ...
      val result = withRangeData(5) { table =>
        Await.result(table.query(pk = "5", rk.between("2", "3")).compile.to(List).unsafeToFuture().throwErrors, TEST_TIMEOUT)
      }
    ...
    it("should compile startswith") {
      ...
      val result = withRangeData(13) { table =>
        Await.result(table.query(pk = "13", rk.beginsWith("1")).compile.to(List).unsafeToFuture().throwErrors, TEST_TIMEOUT)
      }
    ...
    it("should work without rk condition") {
      ...
      val result = withRangeData(13) { table =>
        Await.result(table.query(pk = "13").compile.to(List).unsafeToFuture().throwErrors, TEST_TIMEOUT)
      }
    ...
    it("should work with a lots of data") {
      ...
      val result = withRangeData(1000, payload = Some(payload)) { table =>
        Await.result(table.query(pk = "1000").compile.to(List).unsafeToFuture().throwErrors, TEST_TIMEOUT)
      }
