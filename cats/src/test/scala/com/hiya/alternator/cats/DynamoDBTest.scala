  ...
  describe("putWhen") {
    ...
    it("should work for optimistic locking") {
      DataPK.config.withTable(client) { table =>
        import com.hiya.alternator.syntax.cond._
        Await.result(table.putWhen(DataPK("new", 1000), attr("key").notExists).unsafeToFuture(), TEST_TIMEOUT) shouldBe true
        Await.result(table.putWhen(DataPK("new", 1001), attr("value") === 1000).unsafeToFuture(), TEST_TIMEOUT) shouldBe true
        Await.result(table.putWhen(DataPK("new", 1001), attr("value") === 1000).unsafeToFuture(), TEST_TIMEOUT) shouldBe false
      }
    }
  }
