...
class GenericTest extends AnyFunSpec with Matchers with ScalaCheckDrivenPropertyChecks {
  ...
  private def testReadWrite[A: DynamoFormat: TypeTag](
    gen: Gen[A],
    option: Boolean = true,
    list: Boolean = true
  ): Unit = {
...
object GenericTest {
  ...
  sealed trait B
  final case class B0(i: Int, l: Long, bd: BigDecimal, f: Float, d: Double, s: String, b: Boolean, ba: ByteBuffer)
    extends B
  final case class B1(
    i: Option[Int],
    l: Option[Long],
