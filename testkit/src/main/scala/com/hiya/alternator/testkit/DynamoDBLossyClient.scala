...
class DynamoDBLossyClient(stableClient: DynamoDbAsyncClient)(implicit ec: ExecutionContext)
  ...
  override def batchWriteItem(
    ...
    Random.nextDouble() match {
      case p if p < 0.2 =>
        Future
          .failed[BatchWriteItemResponse](ProvisionedThroughputExceededException.builder().build())
          .toJava
          .toCompletableFuture
      case p if p < 0.9 =>
        val (ok, fail) =
  ...
  override def batchGetItem(batchGetItemRequest: BatchGetItemRequest): CompletableFuture[BatchGetItemResponse] = {
    Random.nextDouble() match {
      case p if p < 0.2 =>
        Future
          .failed[BatchGetItemResponse](ProvisionedThroughputExceededException.builder().build())
          .toJava
          .toCompletableFuture
      case p if p < 0.9 =>
        val (ok, fail) =
